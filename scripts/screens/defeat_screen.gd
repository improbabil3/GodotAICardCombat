...
@onready var _defeat_label: Label = $CenterContainer/ContentPanel/ContentMargin/VBox/DefeatLabel
...
func _ready() -> void:
	...
	var current_enemy := GameManager.get_current_enemy()
	var current_enemy_name := current_enemy.enemy_name if current_enemy != null else "Nemico Sconosciuto"
	_populate_portraits(current_enemy_name)
	_run_meta_label.text = "Pilota: %s   |   HP finale: %s   |   Avversario: %s" % [
		_player_name.text,
...
func _apply_responsive_layout() -> void:
	...
	_defeat_label.add_theme_font_size_override("font_size", 40 if compact_touch_ui else 52)
